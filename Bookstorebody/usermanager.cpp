...
void Usersystem::UserSelectBook(int book_index,string isbn) {
    user_select_[user_select_.size() - 1].index_book = book_index;
    user_select_[user_select_.size() - 1].bookselected_ = true;
    strcpy(user_select_[user_select_.size() - 1].isbn_, isbn.c_str());
}

string Usersystem::Bookisbn() {
    return user_select_[user_select_.size() - 1].isbn_;
}
string Usersystem::StaffNow() {
    if(user_select_.empty()) return "";
    return user_select_[user_select_.size() - 1].ID;
}